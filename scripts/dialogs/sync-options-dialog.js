...
export class SyncOptionsDialog extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  _getFolderIdByName(name, type) {
    ...
    try {
      ...
      const found = folders.find(f => String(f.name || '').toLowerCase() === n.toLowerCase());
  ...
  _getPreferredActorFolderId(kind) {
    ...
    const arr =
      cfg?.includeRules?.filters?.actors?.includeFolders?.[kind === 'pc' ? 'pcs' : 'npcs'];
  ...
  _toApiItemPayload(item, worldId) {
    ...
    const name = String(get(m.namePath || 'name', item?.name || 'Item') || 'Item');
  ...
  async _writeItemFromApi(item, source) {
    const cfg = this._getImportConfig();
    const m = cfg?.itemMappings || {};
    const descriptionHtml = Utils.markdownToStoredHtml(source?.description || '');
    const updates = {};
      ...
      syncProgress: {
        ...
        progressPercentage:
          this.syncProgress.total > 0
            ? Math.round((this.syncProgress.processed / this.syncProgress.total) * 100)
            : 0,
      },
    ...
    for (const type of actorTypes) {
      try {
        if (game.system?.model?.Actor?.[type]) {
          ...
          props.forEach(p => {
            const systemPath = p.path.startsWith('system.') ? p.path : `system.${p.path}`;
  ...
  _getItemStringProperties() {
    ...
    // 2. Try system data model templates
    const itemTypes = ['weapon', 'equipment', 'consumable', 'spell', 'feat', 'loot'];
    ...
    const knownPaths = [
      ...
      { path: 'system.unidentified.description', preview: 'Unidentified description' },
    ];
    knownPaths.forEach(p => {
      if (!allProperties.has(p.path)) allProperties.set(p.path, p);
    });
  ...
  _onRender(context, options) {
    ...
    if (!this._hooksBound) {
      ...
      const invalidateAndMaybeReload = tabName => {
        try {
          ...
          if (this._tabLoaded && tabName in this._tabLoaded) this._tabLoaded[tabName] = false;
          // If the tab is currently active, reload immediately
          if (this.activeTab === tabName) this._loadRowsForTab(tabName)?.catch?.(() => {});
        } catch (_) {
          /* no-op */
        }
      };
    ...
    if (!html.dataset.rowHandlersBound) {
      html.addEventListener('click', async e => {
        ...
        if (rowActorPull) {
          e.stopPropagation();
          return this._pullSingleCharacterByArchivistId(rowActorPull.dataset.archivistId);
        }
        if (rowItemPush) {
          e.stopPropagation();
        ...
        if (rowItemPull) {
          e.stopPropagation();
          return this._pullSingleItemByArchivistId(rowItemPull.dataset.archivistId);
        }
        if (rowFactionPush) {
          e.stopPropagation();
        ...
        if (rowFactionPull) {
          e.stopPropagation();
          return this._pullSingleFactionByArchivistId(rowFactionPull.dataset.archivistId);
        }
        if (rowLocationPush) {
          e.stopPropagation();
        ...
        if (rowLocationPull) {
          e.stopPropagation();
          return this._pullSingleLocationByArchivistId(rowLocationPull.dataset.archivistId);
        }
        if (rowRecapPush) {
          e.stopPropagation();
    ...
    html.querySelector('.cfg-realtime-sync')?.addEventListener('change', async event => {
      ...
      try {
        ...
        ui.notifications.info(enabled ? 'Real-Time Sync enabled' : 'Real-Time Sync disabled');
      } catch (e) {
  ...
  async _pullSingleCharacterByArchivistId(archivistId) {
    try {
      ...
      const existing = (game.actors?.contents ?? game.actors ?? []).find(
        a => String(a.getFlag(CONFIG.MODULE_ID, 'archivistId')) === String(archivistId)
      );
      ...
      const descPath =
        cfg?.actorMappings?.[actorType]?.descriptionPath || 'system.details.biography.value';
      if (existing) {
  ...
  async _pullSingleItemByArchivistId(archivistId) {
    try {
      ...
      const existing = (game.items?.contents ?? game.items ?? []).find(
        i => String(i.getFlag(CONFIG.MODULE_ID, 'archivistId')) === String(archivistId)
      );
      const cfg = settingsManager.getImportConfig();
    ...
    try {
      ...
      for (const c of apiChars) {
        if (!linkedIds.has(String(c.id))) {
          ...
          const imageUrl =
            typeof c.image === 'string' && c.image.trim().length ? c.image.trim() : null;
          rows.push({
            type: 'character',
            name,
  ...
  _buildPreviewData(sample) {
    ...
    for (const r of sample) {
      ...
      const defaultOptions = flattened
        .slice(0, 24)
        .map(({ path, value }) => ({ path, label: `${path} — ${String(value).slice(0, 40)}` }));
      const fieldOverrides = corrections?.byUuid?.[uuid]?.fieldPaths || {};
      const fieldRows = fields.map(f => {
        const selectedPath = fieldOverrides[f.key] || '';
        const options = [{ path: '', label: '(auto)' }, ...defaultOptions].map(o => ({
          ...o,
          selected: o.path === selectedPath,
        }));
      ...
      const charType =
        corrected?.characterType || (labelSet.has('PC') ? 'PC' : labelSet.has('NPC') ? 'NPC' : '');
  ...
  async _refreshImporterPreview() {
    ...
    const data =
      this.importerViewMode === 'all' ? importerService.all() : importerService.sample(sampleSize);
  ...
  async _onCreateWorld(event) {
    ...
    try {
      ...
      this.syncInProgress = true;
      this.render();
      const res = await archivistApi.createCampaign(settingsManager.getApiKey(), {
        title,
        description,
      });
    ...
    try {
      ...
      if (response.success) {
        ...
        ui.notifications.error(
          response.message || game.i18n.localize('ARCHIVIST_SYNC.errors.fetchFailed')
        );
      }
    } catch (error) {
  ...
  async _onSyncTitle(event) {
    ...
    try {
      ...
      if (response.success) {
        ...
        ui.notifications.error(
          response.message || game.i18n.localize('ARCHIVIST_SYNC.errors.syncFailed')
        );
      }
    } catch (error) {
  ...
  async _onSyncToArchivist(event) {
    ...
    try {
      ...
      try {
        const titleData = { title: game.world.title, description: game.world.description || '' };
      ...
      for (const c of apiItems) {
        ...
        if (existingActor) {
          try {
            ...
            if (bio) {
              await writeBestBiography(existingActor, Utils.markdownToStoredHtml(bio));
            }
            updatedCount++;
          } catch (e) {
          ...
          try {
            ...
            try {
              const folderId = this._getPreferredActorFolderId(type === 'character' ? 'pc' : 'npc');
          ...
          if (created) {
            ...
            if (bio) {
              await writeBestBiography(created, Utils.markdownToStoredHtml(bio));
            }
          }
        }
      }
  ...
  async _onPushFactions(event) {
    ...
    try {
      ...
      ui.notifications.info(
        game.i18n.localize('ARCHIVIST_SYNC.messages.factionsSynced') + ` (${res.count})`
      );
    } catch (error) {
  ...
  async _onPushLocations(event) {
    ...
    try {
      ...
      ui.notifications.info(
        game.i18n.localize('ARCHIVIST_SYNC.messages.locationsSynced') + ` (${res.count})`
      );
    } catch (error) {
  ...
  _fillWizardFields(root, cfg) {
    ...
    if (itemsSelect) {
      itemsSelect.value = cfg?.includeRules?.filters?.items?.includeActorOwnedFrom || 'pc';
    }
  ...
  async _onWizardTest(event) {
    ...
    const value = path
      ? path.split('.').reduce((o, k) => (o == null ? undefined : o[k]), firstPc)
      : undefined;
    const preview =
  ...
  async _onPullItems(event) {
    ...
    try {
      ...
      const itemTypes =
        ...
        Item?.metadata?.types ||
        [];
      const defaultType = itemTypes.includes('loot') ? 'loot' : itemTypes[0] || 'loot';
      ...
      for (const it of apiItems) {
        ...
        const imageUrl =
          typeof it.image === 'string' && it.image.trim().length ? it.image.trim() : null;
        const found = existing.get(it.id);
        if (found) {
          await this._writeItemFromApi(found, { name, description, image: imageUrl });
          updatedCount += 1;
        } else {
          ...
          if (created) {
            ...
            try {
              await created.setFlag(CONFIG.MODULE_ID, 'archivistId', it.id);
              await created.setFlag(CONFIG.MODULE_ID, 'archivistWorldId', worldId);
            } catch (_) {
              /* ignore */
            }
            await this._writeItemFromApi(created, { name, description, image: imageUrl });
          }
        }
      }

      ui.notifications.info(`Items pulled: ${createdCount} created, ${updatedCount} updated`);
      this._tabLoaded.items = false;
      await this._loadItemRows?.();
    } catch (e) {
  ...
  async _pushSingleFactionPage(pageId) {
    try {
      ...
      const payload = { name, description: toMarkdownIfHtml(cleaned), campaign_id: worldId };
      this.syncInProgress = true;
      this.render();
      if (meta?.id) await archivistApi.updateFaction(apiKey, meta.id, payload);
      else {
        const created = await archivistApi.createFaction(apiKey, payload);
        if (created?.success && created?.data?.id) {
          await Utils.setPageArchivistMeta(p, created.data.id, 'faction', worldId);
        }
      }
  ...
  async _pushSingleLocationPage(pageId) {
    try {
      ...
      if (meta?.id) await archivistApi.updateLocation(apiKey, meta.id, payload);
        ...
        const created = await archivistApi.createLocation(apiKey, payload);
        if (created?.success && created?.data?.id) {
          await Utils.setPageArchivistMeta(p, created.data.id, 'location', worldId);
        }
      }
  ...
  async _pullSingleRecap(sessionId) {
    try {
      ...
      try {
        await page.setFlag(CONFIG.MODULE_ID, 'sessionDate', String(s.session_date));
      } catch (_) {}
