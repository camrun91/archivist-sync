...
import {
  ...
  mapItemToArchivist,
  mapJournalToFaction,
} from '../modules/deterministic-mapper.js';
...
export class ImporterService {
  ...
  async saveCorrections(corrections) {
    ...
    await settingsManager.setMappingOverride(
      JSON.stringify({ byKey: normalized.byKey || {}, byUuid: normalized.byUuid || {} })
    );
    return true;
  }
  ...
  all() {
    ...
    if (!settingsManager.getSemanticMappingEnabled()) {
      ui.notifications?.warn?.(
        'Semantic importer is disabled. Use the Mapping Wizard tab to run deterministic sync.'
      );
    ...
    for (const e of entities) {
      try {
        ...
        if (score >= thresholdA) {
          ...
          const res = await upsertMappedEntity(apiKey, worldId, srcDoc, {
            ...corrected,
            forceCreate: true,
          });
          if (res && res.success) {
            ...
            try {
              await srcDoc?.setFlag?.(settingsManager.moduleId, 'archivistFingerprint', fp);
            } catch (_) {}
  ...
  async pushFiltered(filter) {
    ...
    for (const e of entities) {
      ...
      if (
        filter?.folderMatch &&
        e.folderName &&
        !new RegExp(filter.folderMatch, 'i').test(e.folderName)
      ) {
        continue;
      }
      const srcDoc = await fromUuid(e.sourcePath);
  ...
  async pushDeterministic(progressCallback = null) {
    ...
    const safeProgress = payload => {
      ...
      try {
        progressCallback.updateSyncProgress(payload);
      } catch (_) {
        /* ignore UI errors */
      }
    };
    ...
    const actorPool = [
      ...pcs,
      ...(config?.includeRules?.filters?.items?.includeActorOwnedFrom === 'pc+npc'
        ? enabledNpcs
        : []),
    ];
    ...
    const processEntity = async (entity, mapper, type) => {
      ...
      try {
        ...
        if (res?.success) {
          ...
          console.warn(
            `${CONFIG.MODULE_TITLE} | Failed to sync ${type}: ${entityName} - ${res?.message || 'Unknown error'}`
          );
        ...
        console.error(
          `${CONFIG.MODULE_TITLE} | Exception processing ${type}: ${entityName}`,
          error
        );
        failedEntities.push({
          entity,
    ...
    if (config?.actorMappings?.pc?.enabled !== false) {
      ...
      for (let i = 0; i < pcs.length; i++) {
        ...
        if ((i + 1) % 10 === 0 && i < pcs.length - 1) {
          console.log(
            `${CONFIG.MODULE_TITLE} | Processed ${i + 1}/${pcs.length} PCs, pausing briefly...`
          );
    ...
    if (config?.actorMappings?.npc?.enabled) {
      ...
      for (let i = 0; i < npcs.length; i++) {
        ...
        if ((i + 1) % 10 === 0 && i < npcs.length - 1) {
          console.log(
            `${CONFIG.MODULE_TITLE} | Processed ${i + 1}/${npcs.length} NPCs, pausing briefly...`
          );
          await new Promise(r => setTimeout(r, 1000));
        }
      }
    ...
    for (let i = 0; i < usedItems.length; i++) {
      ...
      if ((i + 1) % 10 === 0 && i < usedItems.length - 1) {
        console.log(
          `${CONFIG.MODULE_TITLE} | Processed ${i + 1}/${usedItems.length} items, pausing briefly...`
        );
        await new Promise(r => setTimeout(r, 1000));
      }
    }
    ...
    for (let i = 0; i < factions.length; i++) {
      ...
      if ((i + 1) % 10 === 0 && i < factions.length - 1) {
        console.log(
          `${CONFIG.MODULE_TITLE} | Processed ${i + 1}/${factions.length} factions, pausing briefly...`
        );
        await new Promise(r => setTimeout(r, 1000));
      }
    }
    ...
    if (retryable.length > 0) {
      console.log(
        `${CONFIG.MODULE_TITLE} | Retrying ${retryable.length} failed entities after brief delay...`
      );
      ...
      for (const failedItem of retryable) {
        console.log(
          `${CONFIG.MODULE_TITLE} | Retrying ${failedItem.type}: ${failedItem.entity.name || 'Unknown'}`
        );
        const success = await processEntity(
          failedItem.entity,
          failedItem.mapper,
          failedItem.type + ' (retry)'
        );
        if (success) {
    ...
    console.log(
      `${CONFIG.MODULE_TITLE} | Sync complete: ${count} succeeded, ${failed} failed out of ${totalProcessed} total`
    );

    if (failedEntities.length > 0) {
      console.warn(
        `${CONFIG.MODULE_TITLE} | Failed entities:`,
        failedEntities.map(f => `${f.type}: ${f.entity.name || 'Unknown'} (${f.error})`)
      );
    }
