...
Hooks.once('init', function () {
  try { console.log('[Archivist Sync] init'); } catch (_) { }
...
Hooks.once('setup', function () {
  try { console.log('[Archivist Sync] setup'); } catch (_) { }
...
Hooks.once('ready', async function () {
  ...
  try { console.log('[Archivist Sync] after availability update', { activeTab: ui.sidebar?.activeTab, hasSidebar: !!ui.sidebar }); } catch (_) { }

  // Delegated renderer: when the archivist tab button is clicked, render chat into panel
  try {
    const onClick = (ev) => {
      const btn = ev.target && (ev.target.closest?.('#sidebar [data-action="tab"][data-tab="archivist-chat"]'));
      if (!btn) return;
      ...
      setTimeout(async () => {
        try {
          ...
          console.log('[Archivist Sync] Post-click render attempt', { hasPanel: !!panel, activeTab: ui.sidebar?.activeTab, expanded: ui.sidebar?._expanded });
          if (!panel) return;
          // Ensure this panel is visible/active even if core didn't switch
          try {
            const contentWrap = panel.parentElement;
            contentWrap?.querySelectorAll?.('.tab').forEach(el => { el.classList.remove('active'); el.style.display = 'none'; });
            panel.style.display = '';
          ...
          if (!window.__ARCHIVIST_SIDEBAR_CHAT__) window.__ARCHIVIST_SIDEBAR_CHAT__ = new AskChatWindow({ popOut: false });
        ...
        } catch (e) { console.warn('[Archivist Sync] Delegated render failed', e); }
      }, 0);
    };
    document.addEventListener('click', onClick, true);
  } catch (e) { console.warn('[Archivist Sync] Failed to install delegated renderer', e); }
  ...
  try {
    const onOtherTabClick = (ev) => {
      ...
      setTimeout(() => {
        try {
          const sidebar = document.getElementById('sidebar');
          const contentWrap = sidebar?.querySelector?.('#sidebar-content, section.content, .content') || sidebar?.querySelector('section.tab, .tab')?.parentElement;
          if (contentWrap) {
            // Remove inline display overrides so core can manage visibility
            contentWrap.querySelectorAll('.tab').forEach(el => { el.style.display = ''; });
        ...
        } catch (e) { console.warn('[Archivist Sync] Failed clearing overrides', e); }
      }, 0);
    };
    document.addEventListener('click', onOtherTabClick, true);
  } catch (e) { console.warn('[Archivist Sync] Failed to install delegated cleanup', e); }
  ...
  } catch (e) { console.warn('[Archivist Sync] Failed to install Real-Time Sync listeners', e); }
...
function updateArchivistChatAvailability() {
  ...
  try { console.log('[Archivist Sync] updateArchivistChatAvailability()', { isAvailable }); } catch (_) { }
  ...
  if (isAvailable) {
    ...
    if (sidebar) {
      const tabsNav = sidebar.querySelector('#sidebar-tabs, nav.tabs');
      let tabButton = sidebar.querySelector('[data-tab="archivist-chat"]');
      ...
      if (!tabButton && tabsNav) {
        try {
          ...
          btn.setAttribute('aria-label', game.i18n?.localize?.('ARCHIVIST_SYNC.Menu.AskChat.Label') || 'Archivist Chat');
          btn.setAttribute('data-tooltip', game.i18n?.localize?.('ARCHIVIST_SYNC.Menu.AskChat.Label') || 'Archivist Chat');
          ...
          btn.addEventListener('click', async (ev) => {
            ev.preventDefault();
            try { console.log('[Archivist Sync] Sidebar button click'); } catch (_) { }
            const isActive = ui.sidebar?.activeTab === 'archivist-chat';
            const isExpanded = ui.sidebar?._expanded;
            try { console.log('[Archivist Sync] click state', { isActive, isExpanded }); } catch (_) { }
            if (isActive && isExpanded) {
              try { ui.sidebar?.collapse?.(); } catch (_) { }
            } else {
              try { ui.sidebar?.expand?.(); } catch (_) { }
              try { ui.sidebar?.changeTab?.('archivist-chat'); } catch (_) { }
              // Ensure panel exists and render the chat UI as a fallback
              const sb = document.getElementById('sidebar');
              let panel = sb?.querySelector?.('#archivist-chat.tab');
              if (!panel) {
                const contentWrap = sb?.querySelector('#sidebar-content, section.content, .content') || sb?.querySelector('section.tab, .tab')?.parentElement;
                if (contentWrap) {
              ...
              if (panel) {
                try {
                  ...
                  window.__ARCHIVIST_SIDEBAR_CHAT__._mountEl = panel;
                  await window.__ARCHIVIST_SIDEBAR_CHAT__.render(false);
                } catch (e) { console.warn('[Archivist Sync] Fallback chat render failed', e); }
              }
            }
          });
          li.appendChild(btn);
          ...
          menu.appendChild(li);
        } catch (e) { console.warn('[Archivist Sync] Failed to inject fallback Sidebar tab button', e); }
      }
    }
    // Re-render to reflect visibility changes
    try { ui.sidebar?.render?.({ force: true }); } catch (e) { console.warn('[Archivist Sync] Sidebar render failed', e); }
  } else {
...
function installRealtimeSyncListeners() {
  ...
  const toItemPayload = (item) => {
    ...
    return { name, description: Utils.toMarkdownIfHtml?.(desc) || desc, ...(image ? { image } : {}), campaign_id: worldId };
  };
  ...
  const toFactionPayload = (page) => {
    ...
    return { name, description: Utils.toMarkdownIfHtml?.(cleaned) || cleaned, campaign_id: worldId };
  };
  ...
  Hooks.on('createActor', async (doc) => {
    try {
      ...
      if (res?.success && res?.data?.id) await doc.setFlag(CONFIG.MODULE_ID, 'archivistId', res.data.id);
    } catch (e) { console.warn('[RTS] createActor failed', e); }
  });
  Hooks.on('createItem', async (doc) => {
    ...
    } catch (e) { console.warn('[RTS] createItem failed', e); }
  });
  ...
  Hooks.on('createJournalEntryPage', async (page) => {
    try {
      ...
      if (isFactionPage(page)) {
        ...
        if (res?.success && res?.data?.id) await Utils.setPageArchivistMeta(page, res.data.id, 'faction', worldId);
      } else if (isLocationPage(page)) {
        const res = await archivistApi.createLocation(apiKey, toLocationPayload(page));
        if (res?.success && res?.data?.id) await Utils.setPageArchivistMeta(page, res.data.id, 'location', worldId);
      }
    } catch (e) { console.warn('[RTS] createJournalEntryPage failed', e); }
  });
  ...
  Hooks.on('updateActor', async (doc, changes) => {
    ...
    } catch (e) { console.warn('[RTS] updateActor failed', e); }
  });
  Hooks.on('updateItem', async (doc, changes) => {
    ...
    } catch (e) { console.warn('[RTS] updateItem failed', e); }
  });
  Hooks.on('updateJournalEntryPage', async (page, changes) => {
    try {
      ...
      if (!meta?.id) return;
      if (isFactionPage(page)) await archivistApi.updateFaction(apiKey, meta.id, toFactionPayload(page));
      else if (isLocationPage(page)) await archivistApi.updateLocation(apiKey, meta.id, toLocationPayload(page));
      else if (isRecapPage(page)) {
        // Recaps: update session summary/title only; do not create/delete
        const title = page.name; const html = Utils.extractPageHtml(page);
        await archivistApi.updateSession(apiKey, meta.id, { title, summary: Utils.toMarkdownIfHtml?.(html) || html });
      }
    } catch (e) { console.warn('[RTS] updateJournalEntryPage failed', e); }
  });
  ...
  Hooks.on('preDeleteActor', async (doc) => {
    ...
    } catch (e) { console.warn('[RTS] preDeleteActor failed', e); }
  });
  Hooks.on('preDeleteItem', async (doc) => {
    ...
    } catch (e) { console.warn('[RTS] preDeleteItem failed', e); }
  });
  Hooks.on('preDeleteJournalEntryPage', async (page) => {
    try {
      ...
      if (isFactionPage(page) && archivistApi.deleteFaction) await archivistApi.deleteFaction(apiKey, meta.id);
      if (isLocationPage(page) && archivistApi.deleteLocation) await archivistApi.deleteLocation(apiKey, meta.id);
    } catch (e) { console.warn('[RTS] preDeleteJournalEntryPage failed', e); }
  });
}