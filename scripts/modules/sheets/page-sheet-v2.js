...
import {
  ...
  unlinkDocs,
  setLocationParent,
} from '../../modules/links/helpers.js';
import { Utils } from '../../modules/utils.js';

const V2 = foundry.applications.api;

class ArchivistBasePageSheetV2 extends V2.HandlebarsApplicationMixin(
  V2.DocumentSheetV2
) {
  constructor(options = {}) {
    super(options);
  ...
  async _prepareContext(_options) {
    console.log('[Archivist V2 Sheet] _prepareContext START', {
      editMode: this._editMode,
      editingInfo: this._editingInfo,
    });
    ...
    try {
      ...
      if (sessionDate) {
        ...
        try {
          ...
          if (!isNaN(d.getTime())) {
            sessionDateFormatted = d.toLocaleDateString('en-US', {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit',
            });
    ...
    console.log('[Archivist V2 Sheet] _prepareContext END', {
      editMode: this._editMode,
      editingInfo: result.setup.editingInfo,
      sheetType: result.setup.sheetType,
    });
    return result;
  }
  ...
  _onRender(context, options) {
    ...
    try {
      ...
      if (!game.user?.isGM) {
        const notesTabLink = root.querySelector(
          '.archivist-nav .item[data-tab="notes"]'
        );
      ...
      try {
        if (this._editingInfo) {
          ...
          root
            .querySelectorAll(
              '.pm-menu [data-action="save"], .prosemirror [data-action="save"], .ProseMirror-menu [data-action="save"]'
            )
            .forEach((btn) => {
      ...
      if (game.user?.isGM) {
        try {
          ...
          console.log(
            '[Archivist V2 Sheet] Edit toggle button found:',
            !!toggleBtn
          );
          if (toggleBtn && !toggleBtn.dataset.bound) {
            ...
            console.log(
              '[Archivist V2 Sheet] Edit toggle button listener bound'
            );
          }
          ...
          if (visBtn && !visBtn.dataset.bound) {
            visBtn.addEventListener('click', async (ev) => {
              ...
              try {
                ...
                if (i)
                  i.className =
                    next >= OBS ? 'fa-solid fa-eye' : 'fa-solid fa-eye-slash';
              } catch (_) {}
            });
      ...
      try {
        ...
        const setActive = (id) => {
          root
            .querySelectorAll('.archivist-nav .item')
            .forEach((a) => a.classList.toggle('active', a.dataset.tab === id));
          tabs.forEach((s) =>
      ...
      root.addEventListener('click', (ev) => {
        ...
        if (target) {
          const entry =
            target.documentName === 'JournalEntryPage' ? target.parent : target;
          entry?.sheet
            ?.render?.({ force: true })
            .then((app) => app?.bringToFront?.());
        }
      });

      // Handle GM Notes ProseMirror save events
      try {
        const gmNotesEditor = root.querySelector(
          'prose-mirror[name="gmNotes"]'
        );
        if (gmNotesEditor && game.user?.isGM) {
          ...
          gmNotesEditor.addEventListener('save', async (event) => {
            ...
            try {
              const value = Array.isArray(gmNotesEditor.value)
                ? gmNotesEditor.value[0]
                : gmNotesEditor.value;
      ...
      root.addEventListener('click', async (ev) => {
        ...
        try {
          if (target) {
            ...
            const flags = this._getArchivistFlags();
            const arr =
              (flags.archivistRefs && flags.archivistRefs[bucket]) || [];
          ...
          try {
            ...
            const fromId = String(
              fromFlags.archivistId || this.document.id || ''
            );
            if (apiKey && campaignId && fromId && aid) {
              const list = await archivistApi.listLinksByFromId(
                apiKey,
                campaignId,
                fromId
              );
              ...
              const toDelete = links.filter(
                (l) => String(l?.to_id || l?.toId || '') === String(aid)
              );
              for (const L of toDelete) {
                const linkId = L?.id || L?._id || L?.link_id || L?.linkId;
                if (!linkId) continue;
                await archivistApi.deleteLink(
                  apiKey,
                  campaignId,
                  String(linkId)
                );
      ...
      root.addEventListener('click', async (ev) => {
        try {
          const unlinkBtn = ev.target?.closest?.(
            '[data-action="unlink-foundry-journal"]'
          );
          if (unlinkBtn) {
            if (!game.user?.isGM) return;
            ev.preventDefault();
            ev.stopPropagation();
            const ref =
              unlinkBtn.dataset.journalRef || unlinkBtn.dataset.journalId;
            if (!ref) return;
            const flags = this._getArchivistFlags();
            const next = {
              ...(flags.foundryRefs || {
            ...
            next.journals = (
              Array.isArray(next.journals) ? next.journals : []
            ).filter((x) => String(x) !== String(ref));
          ...
          try {
            ...
            if (!scene) {
              ui.notifications?.warn(
                'No active scene found. Please activate a scene first.'
              );
              return;
            }
            ...
            if (!actor) {
              ui.notifications?.error(
                'No linked actor found for this character.'
              );
              return;
            }
            ...
            ui.notifications?.info(
              `${actor.name} has been placed on the scene.`
            );
        ...
        root.querySelectorAll('prose-mirror').forEach((editor) => {
          ...
          editor.addEventListener('save', async (event) => {
            ...
            try {
              ...
              console.warn(
                '[Archivist Sync][PageV2] ProseMirror save failed',
                e
              );
            }
          });
      ...
      try {
        ...
        if (actorTarget) {
          if (!game.user?.isGM) {
            ...
            actorTarget.addEventListener('dragover', (ev) =>
              ev.preventDefault()
            );
        ...
        if (itemTarget) {
          if (!game.user?.isGM) {
            ...
            itemTarget.addEventListener('dragover', (ev) =>
              ev.preventDefault()
            );
  ...
  async _toggleEditMode() {
    console.log(
      '[Archivist V2 Sheet] _toggleEditMode called, current editMode:',
      this._editMode
    );
    if (!this._editMode) {
  ...
  async _commitEdits() {
    ...
    try {
      const titleInput = this.element?.querySelector?.(
        '.archivist-title-input'
      );
      console.log(
        '[Archivist V2 Sheet] Title input found:',
        !!titleInput,
        titleInput?.value
      );
      if (titleInput) {
    ...
    try {
      ...
      const textarea = root?.querySelector?.(
        'textarea.archivist-info-textarea'
      );
      if (textarea) {
      ...
      console.warn(
        '[Archivist Sync][V2] commit: failed updating page content',
        e
      );
    }
    ...
    if (sheetType === 'recap' || sheetType === 'session') {
      try {
        ...
        console.log(
          '[Archivist V2 Sheet] Recap date input found:',
          !!dateEl,
          dateEl?.value
        );
        ...
        if (sessionDate) {
          ...
          try {
            const pFlags =
              infoPage?.getFlag?.(CONFIG.MODULE_ID, 'sessionDate') || '';
            const eFlags =
              entry?.getFlag?.(CONFIG.MODULE_ID, 'sessionDate') || '';
            originalIso = String(pFlags || eFlags || '').trim();
          } catch (_) {}
          const originalTime = originalIso.includes('T')
            ? originalIso.split('T')[1]
            : '00:00:00';
          const recombined = `${sessionDate}T${originalTime}`;
          // Write both page and entry flags for robustness
          try {
            await infoPage?.setFlag?.(
              CONFIG.MODULE_ID,
              'sessionDate',
              recombined
            );
          ...
          try {
            await entry?.setFlag?.(CONFIG.MODULE_ID, 'sessionDate', recombined);
          } catch (_) {}
        }
      } catch (_) {}
    }
    ...
    try {
      console.log('[Archivist V2 Sheet] Remote sync check:', {
        apiKey: !!apiKey,
  ...
  async _onArchivistDrop(event) {
    ...
    try {
      ...
      const droppedFlags =
        dropped?.getFlag?.(CONFIG.MODULE_ID, 'archivist') ||
        dropped?.parent?.getFlag?.(CONFIG.MODULE_ID, 'archivist') ||
        {};
      ...
      if (dropped.documentName === 'Actor' || dropped.documentName === 'Item') {
        ...
        try {
          ...
          if (!isArchivistSheet) {
            ...
            const arr = Array.isArray(flags.foundryRefs.journals)
              ? flags.foundryRefs.journals.slice()
              : [];
  ...
  _bucketForDrop(doc) {
    ...
    if (docName === 'JournalEntry' || docName === 'JournalEntryPage') {
      const dFlags =
        doc?.getFlag?.(CONFIG.MODULE_ID, 'archivist') ||
        doc?.parent?.getFlag?.(CONFIG.MODULE_ID, 'archivist') ||
        {};
      const dType = String(dFlags.sheetType || '').toLowerCase();
      if (dType === 'character' || dType === 'pc' || dType === 'npc')
        return 'characters';
    ...
    try {
      if (page?.type === 'text') {
        ...
        if (fmt === 2 && typeof md === 'string') {
          ...
          console.log(
            `[Sheet] Converted markdown to HTML, length:`,
            html.length
          );
          return html;
        }
    ...
    try {
      ...
      const byActorId = (id) =>
        (game.actors?.contents || []).find(
          (a) => a.getFlag(CONFIG.MODULE_ID, 'archivistId') === id
        );
      const byItemId = (id) =>
        (game.items?.contents || []).find(
          (i) => i.getFlag(CONFIG.MODULE_ID, 'archivistId') === id
        );
        ...
        try {
          const a =
            root.parentElement?.querySelector?.(
              `.archivist-nav [data-tab="${tab}"]`
            ) || root.querySelector?.(`.archivist-nav [data-tab="${tab}"]`);
          if (!a) return;
      ...
      const charactersEl = root.querySelector(
        '.tab[data-tab="characters"] .archivist-grid'
      );
      if (charactersEl) {
        charactersEl.innerHTML = '';
        let ids = [];
        try {
          const { linkIndexer } = await import(
            '../../modules/links/indexer.js'
          );
          if (!isCurrent()) return;
          if (!linkIndexer._built) linkIndexer.buildFromWorld();
          const myId =
            this._getArchivistFlags().archivistId || this.document.id;
      ...
      const pcsEl = root.querySelector(
        '.tab[data-tab="pcs"] .archivist-grid, .tab[data-tab="pcs"] .pcs-grid'
      );
      const npcsEl = root.querySelector(
        '.tab[data-tab="npcs"] .archivist-grid, .tab[data-tab="npcs"] .npcs-grid'
      );
      if (pcsEl || npcsEl) {
        let allCharIds = [];
        try {
          ...
          const myId =
            this._getArchivistFlags().archivistId || this.document.id;
          const out = linkIndexer.outboundByFromId.get(myId) || {};
          allCharIds = Array.isArray(out.characters)
            ? out.characters.slice()
            : [];
        } catch (_) {
          allCharIds = [];
        }
        ...
        const classify = (id) => {
          ...
          const sheetType = String(
            (target?.getFlag?.(CONFIG.MODULE_ID, 'archivist') || {})
              .sheetType || ''
          ).toLowerCase();
      ...
      const factionsEl = root.querySelector(
        '.tab[data-tab="factions"] .archivist-grid, .factions-grid'
      );
      if (factionsEl) {
      ...
      const itemsEl = root.querySelector(
        '.tab[data-tab="items"] .archivist-grid, .items-grid'
      );
      if (itemsEl) {
        itemsEl.innerHTML = '';
        let ids = [];
      ...
      // Journals grid
      const journalsGrid = root.querySelector(
        '.tab[data-tab="journals"] .archivist-journals'
      );
      if (journalsGrid) {
        journalsGrid.innerHTML = '';
        const f = this._getArchivistFlags();
        let jRefs = Array.isArray(f?.foundryRefs?.journals)
          ? f.foundryRefs.journals.slice()
          : [];
        jRefs = uniq(jRefs);
        jRefs.sort();
        ...
        for (const ref of jRefs) {
          ...
          if (typeof ref === 'string' && ref.includes('JournalEntryPage.')) {
            ...
            j =
              (game.journal?.contents || []).find((x) => x.id === ref) || null;
          }
          if (j && !canSee(j)) continue;
      ...
      const assocGrid = root.querySelector(
        '.tab[data-tab="locations"] .archivist-grid, .locations-grid'
      );
      if (assocGrid) {
        let ids = [];
        try {
          ...
          const myId =
            this._getArchivistFlags().archivistId || this.document.id;
          const out = linkIndexer.outboundByFromId.get(myId) || {};
          ids = Array.isArray(out.locationsAssociative)
            ? out.locationsAssociative.slice()
            : [];
        } catch (_) {
          ids = [];
        }
      ...
      console.warn(
        '[Archivist Sync][PageV2] _renderLinkedGrids (Base) failed',
        e
      );
    }
  }
        ...
        if (actorId) {
          ...
          if (actor) {
            ...
            const openBtn = card.querySelector(
              '[data-action="open-linked-actor"]'
            );
            const unlinkBtn = card.querySelector(
              '[data-action="unlink-linked-actor"]'
            );
            if (openBtn)
      ...
      if (itemWrap) {
        ...
        if (itemId) {
          ...
          if (itm) {
            ...
            const openBtn = card.querySelector(
              '[data-action="open-linked-item"]'
            );
            const unlinkBtn = card.querySelector(
              '[data-action="unlink-linked-item"]'
            );
            if (openBtn)
              openBtn.addEventListener('click', (ev) => {
                ev.preventDefault();
...
export class EntryPageSheetV2 extends ArchivistBasePageSheetV2 {
  static PARTS = {
    form: { template: 'modules/archivist-sync/templates/sheets/entry.hbs' },
  };
}
export class PCPageSheetV2 extends ArchivistBasePageSheetV2 {
  static PARTS = {
    form: { template: 'modules/archivist-sync/templates/sheets/character.hbs' },
  };
}
export class NPCPageSheetV2 extends ArchivistBasePageSheetV2 {
  static PARTS = {
    form: { template: 'modules/archivist-sync/templates/sheets/character.hbs' },
  };
}
export class CharacterPageSheetV2 extends PCPageSheetV2 {}
export class ItemPageSheetV2 extends ArchivistBasePageSheetV2 {
  static PARTS = {
    form: { template: 'modules/archivist-sync/templates/sheets/item.hbs' },
  };
}
  ...
  _onRender(context, options) {
    ...
    try {
      ...
      try {
        ...
        const current =
          root.querySelector('.archivist-nav .item.active')?.dataset.tab ||
          'info';
        setActive(current);
        nav?.addEventListener('click', (ev) => {
          const a = ev.target?.closest?.('.item[data-tab]');
      ...
      if (parentTarget) {
        if (!game.user?.isGM) {
          ...
          parentTarget.addEventListener('dragover', (ev) =>
            ev.preventDefault()
          );
          parentTarget.addEventListener('drop', async (ev) => {
            ...
            try {
              ...
              const doc = uuid ? await fromUuid(uuid) : null;
              const entry =
                doc?.documentName === 'JournalEntryPage' ? doc.parent : doc;
    ...
    try {
      ...
      if (ancEl) {
        ...
        for (let i = 0; i < full.length; i++) {
          const id = full[i];
          const j = (game.journal?.contents || []).find(
            (x) =>
              (x.getFlag(CONFIG.MODULE_ID, 'archivist') || {}).archivistId ===
              id
          );
      ...
      if (descWrap) {
        ...
        const renderChildren = (parentId, ulNode) => {
          const kids = (linkIndexer.childrenByLocationId.get(parentId) || [])
            .slice()
            .sort();
    ...
    try {
      ...
      if (parentWrap) {
        ...
        if (parentKey) {
          ...
          const parentJ = journals.find((j) => {
            ...
            return (
              (f.archivistId && f.archivistId === parentKey) ||
              j.id === parentKey
            );
          });
          if (parentJ) {
            ...
            card.className = 'mini-card';
            const imgSrc =
              this._resolveSheetImage(parentJ) || 'icons/svg/mountain.svg';
      ...
      if (sceneWrap) {
        ...
        if (sceneId) {
          ...
          if (scene) {
            const thumb =
              scene.thumbnail || scene.img || 'icons/svg/village.svg';
            const card = document.createElement('div');
            card.className = 'mini-card';
...
export class FactionPageSheetV2 extends ArchivistBasePageSheetV2 {
  static PARTS = {
    form: { template: 'modules/archivist-sync/templates/sheets/faction.hbs' },
  };
}
export class RecapPageSheetV2 extends ArchivistBasePageSheetV2 {
  static PARTS = {
...
export function sheetClassId(Cls) {
  return `archivist-sync.${Cls.name}`;
}
