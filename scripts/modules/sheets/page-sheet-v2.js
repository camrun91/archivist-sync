...
class ArchivistBasePageSheetV2 extends V2.HandlebarsApplicationMixin(V2.DocumentSheetV2) {
    ...
    _onRender(context, options) {
        ...
        try {
            ...
            // Send item to player handler (for Item sheets)
            root.addEventListener('click', async ev => {
                ...
                const flags = this._getArchivistFlags();
                const itemId = (flags.foundryRefs?.items || [])[0];
                const item = itemId ? game.items?.get?.(itemId) : null;

                if (!item) {
                    ui.notifications?.error('No linked item found.');
                    return;
                }

                // Get player-owned actors
                const playerActors = game.actors?.filter(a => !a.isToken && a.hasPlayerOwner) || [];
                ...
                // Create a dialog to select which actor(s) to send the item to
                const content = `
                    <form>
                        <div class="form-group">
                            <label>Select Character(s) to receive ${item.name}:</label>
                            <select name="actorId" multiple size="${Math.min(playerActors.length, 10)}">
                                ${playerActors.map(a => `<option value="${a.id}">${a.name}</option>`).join('')}
                            </select>
                        </div>
                    </form>
                `;
                ...
                new Dialog({
                    title: `Send ${item.name} to Player`,
                    content,
                    buttons: {
                        send: {
                            ...
                            label: 'Send',
                            callback: async (html) => {
                                const selectedIds = Array.from(html.find('select[name="actorId"]')[0].selectedOptions)
                                    .map(opt => opt.value);
                                ...
                                for (const actorId of selectedIds) {
                                    ...
                                    if (actor) {
                                        try {
                                            ...
                                            ui.notifications?.info(`${item.name} sent to ${actor.name}.`);
                                        } catch (e) {
                                            console.error('[Archivist Sync][PageV2] send-item failed', e);
                                            ui.notifications?.error(`Failed to send ${item.name} to ${actor.name}.`);
                                        }
                                    }
                                }
                            }
