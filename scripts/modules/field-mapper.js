...
export async function writeBestBiography(actor, htmlString) {
  ...
  const defaultPath =
    settingsManager._defaultDescriptionPath?.(systemId, actorType) ||
  ...
  try {
    ...
    if (/^system\.details\.biography(\.|$)/.test(path)) {
      ...
      if (
        (typeof afterValue === 'string' && afterValue === safe) ||
...
export async function writeBestJournalDescription(journal, htmlString) {
  ...
  if (pagesCollection) {
    ...
    if (!target) {
      target = pages.find(
        p => typeof p?.text?.content === 'string' || typeof p?.text?.markdown === 'string'
      );
    }
    if (target) {
