...
const common = {
    ...
    JournalEntry: [
        { path: 'pages.0.text.content', weight: 100, html: true },
    ],
};
...
export function getCandidates(docType) {
    ...
    if (!slots) {
        console.warn('[Projection.AdapterRegistry] No candidates for', { system: a?.id, docType });
    } else {
        console.log('[Projection.AdapterRegistry] Candidates resolved for', { system: a?.id, docType, count: slots.length });
    }
    return slots;
}
...
Hooks.on('archivist:registerSlot', (systemId, docType, candidate) => {
    try {
        ...
        const rec = registry[systemId] || { id: systemId, slots: { Actor: [], Item: [], Scene: [], JournalEntry: [] } };
