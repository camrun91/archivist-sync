...
export function extractGenericEntities(sampleLimit) {
    ...
    for (const a of actors) {
        ...
        out.push({
            ...
            metadata: { type: a.type, system: a.system, prototypeToken: { disposition: a?.prototypeToken?.disposition } }
        });
    }
    ...
    for (const j of journals) {
        ...
        const content = textContents.length ? textContents.join('\n\n') : unwrapToPlainText(j.content || '');
        const tags = new Set();
    ...
    for (const s of scenes) {
        ...
        out.push({
            ...
            metadata: {
                width: s.width, height: s.height, bg: s?.background,
                grid: {
                    size: s?.grid?.size,
