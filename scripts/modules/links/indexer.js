...
export class LinkIndexer {
  ...
  buildFromWorld() {
    ...
    for (const j of journals) {
      ...
      if (flags.archivistId)
        this.byArchivistId.set(flags.archivistId, { journalId: j.id, flags });
      if (flags.sheetType === 'location') {
        ...
        if (assoc.length)
          this.associatesByLocationId.set(selfKey, assoc.slice());
      }
      ...
      try {
        ...
        const cur = this.outboundByFromId.get(fromId) || {
          characters: [],
          items: [],
          factions: [],
  ...
  async buildFromArchivistAPI() {
    ...
    try {
      ...
      if (!resp?.success) {
        console.warn(
          '[Archivist Sync] LinkIndexer: listLinks failed; falling back to world scan'
        );
        this.buildFromWorld();
        return;
      }
      ...
      const bucketForType = (type) => {
        ...
        if (t === 'entry' || t === 'journal' || t === 'journalentry')
          return 'entries';
        return null;
      };
      ...
      console.warn(
        '[Archivist Sync] LinkIndexer: buildFromArchivistAPI error',
        e
      );
      this.buildFromWorld();
    }
