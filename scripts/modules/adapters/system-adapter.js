    ...
    static actorDescription(actor) {
        ...
        for (const p of candidates) {
            ...
            if (typeof v === 'string' && v.trim()) {
                try { console.log('[SystemAdapter.actorDescription] Selected path', { path: p, length: String(v).length }); } catch (_) { }
                return v;
            }
        }
        try { console.log('[SystemAdapter.actorDescription] No description found for actor'); } catch (_) { }
        return '';
    }
